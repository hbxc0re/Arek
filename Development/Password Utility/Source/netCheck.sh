...
if [ "$CheckZeroStatus" = 0 ] ; then
	...
	if [ $CheckLDAPStatus -lt 255 ] ; then
        	StatusMSG $FUNCNAME "CONFIRMED - Connected to Genentech network" passed
       		StatusMSG $FUNCNAME "Connected to Genentech network" uistatus
...
CheckNet(){
 ...
 if [ $CheckOneStatus != 0 ] || [ $CheckTwoStatus != 0 ] || [ $CheckThreeStatus != 0 ]; then
   	StatusMSG $FUNCNAME "FAILED - Not Connected to Genentech network" error
 	StatusMSG $FUNCNAME "Not Connected to Genentech network" uistatus
	setInstallPercentage $CurrentPercentage.99
	return 1
 else
	StatusMSG $FUNCNAME "CONFIRMED - Connected to Genentech network" passed
