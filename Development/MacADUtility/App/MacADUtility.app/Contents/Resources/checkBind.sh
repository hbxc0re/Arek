...
ldapAttribute(){
		...
		LdapRecord="`$ldapsearch -x -H "ldap://$LdapServer" -D "$UserName@$Domain" -w "$PassWord" -b "$LdapSearchBase" "CN=$UserName*"`"
...
checkCredentials(){
	...
	CheckThree="$(ldapAttribute "$UserName" "$DomainController0")"
